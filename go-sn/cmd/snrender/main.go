...
func main() {
	...
	for _, path := range flag.Args() {
		if err := renderFile(path, *outDir, *quality, *pageNum); err != nil {
			log.Printf("error rendering %s: %v", path, err)
		}
	}
}

func renderFile(path, outDir string, quality, pageNum int) error {
	f, err := os.Open(path)
	if err != nil {
		return err
	}
	...
	for _, p := range pages {
		...
		strokes, err := note.DecodeTotalPath(tp, pageW, pageH)
		if err != nil {
			return fmt.Errorf("page %d DecodeTotalPath: %w", p.Index+1, err)
		}
		log.Printf("page %d: %d strokes", p.Index+1, len(strokes))

		img := note.Render(strokes, pageW, pageH, nil)
